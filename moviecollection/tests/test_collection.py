...
def test_collection_list_get(authenticated_client):
    ...
    assert  response.data.get('data', [])[0].get('title') == collection_field_mapping['title']
...
def test_get_collection_detail(authenticated_client):
    ...
    url = reverse('collection_detail', args=[response.data.get('data', [])[0].get('id')])
    response = authenticated_client.get(url)
    assert response.status_code == status.HTTP_200_OK
...
@pytest.mark.skip
@pytest.mark.django_db
def test_delete_collection_detail(authenticated_client, collection):
    client, user, collection = collection
